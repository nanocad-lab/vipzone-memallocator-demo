...
int main(const int argc, const char *argv[]) {
	...
	for (part = 0; part < 2; part++) {
		...
		if (part == 0) {
			...
			page_array = (unsigned long **) syscall(NR_vip_mmap, NULL, MAX_NUM_PAGES*sizeof(unsigned long *), (PROT_READ | PROT_WRITE | _VIP_TYP_WRITE | _VIP_UTIL_HI), (MAP_ANONYMOUS | MAP_PRIVATE), -1, 0);
		} else {
			printf("Hogging memory using vip_mmap with flags WRITE/LO...\n");
			page_array = (unsigned long **) syscall(NR_vip_mmap, NULL, MAX_NUM_PAGES*sizeof(unsigned long *), (PROT_READ | PROT_WRITE | _VIP_TYP_WRITE | _VIP_UTIL_LO), (MAP_ANONYMOUS | MAP_PRIVATE), -1, 0);
		}

		if (page_array == MAP_FAILED) {
			printf("Error: Could not allocate page array using vip_mmap. Terminating.\n");
			perror("vip_mmap");
			return 1;
		}
		...
		if (part == 0) {
			...
			while ((page < MAX_NUM_PAGES) && (page < MB_to_alloc*1024*1024/PAGE_SIZE) && (*(page_array+page) = (unsigned long *) syscall(NR_vip_mmap, NULL, PAGE_SIZE, (PROT_READ | PROT_WRITE | _VIP_TYP_WRITE | _VIP_UTIL_LO), (MAP_ANONYMOUS | MAP_PRIVATE), -1, 0)) != MAP_FAILED) 
				page++;
			if (*(page_array+page) == MAP_FAILED) {
				printf("Error: Failed to vip_mmap page number %lu, at %0.2f MB.\n", page, (double)page*PAGE_SIZE/(1024*1024));
				perror("vip_mmap");
